...
class MahasiswaController extends Controller
{
    ...
    {
        $data = mahasiswa::all();
        return view('calon_mahasiswa.index', compact('data'));
    }

    public function calon_mahasiswa_detail(Request $request)
    {
        $data = $this->getPendaftarFullDetails($request->id);
        return view('calon_mahasiswa.detail', compact('data'));
    }

    public function calon_mahasiswa_edit(Request $request)
    {
        $id = $request->id;
        $data_utama = mahasiswa::find($id);
        $data_alamat_asal = alamat::find($data_utama->alamat_asal_id);
        ...
        $data['status_saat_mendaftar'] = $this->statusSaatMendaftarTranslator($data_status_saat_mendaftar);
        $data['sumber_informasi'] = $data_sumber_informasi;
        return view('calon_mahasiswa.edit', compact('data'));
    }

    public function calon_mahasiswa_update(Request $request)
    {
        $id = $request->id;
        $data_utama = mahasiswa::find($id);
        $data_alamat_asal = alamat::find($data_utama->alamat_asal_id);
        ...
        $data_utama->save();
        $data = $this->getPendaftarFullDetails($data_utama->id);
        return redirect()->route('calon_mahasiswa.detail', ['id' => $data->id]);
    }

    public function calon_mahasiswa_delete(Request $request)
    {
        $mahasiswa = mahasiswa::find($request->id);
        $pendaftar = pendaftar::where('nomor_pendaftaran', $mahasiswa->nomor_pendaftaran)->get()->first();
        $pendaftar->status = 0;
